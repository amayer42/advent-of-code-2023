...
const calibration_lines = calibration_document.split("\n");
const calibration_values = calibration_lines.map((cl) =>
  get_calibration_value(cl)
);
const calibration_sum = calibration_values.reduce((sum, cv) => sum + cv);

/**
 *
 * @param {string} calibration_line
 * @returns number
 */
function get_calibration_value(calibration_line) {
  /** @type {number | undefined}*/
  let first;
  /** @type {number | undefined}*/
  let last;

  for (let i = 0; i < calibration_line.length; i++) {
    const current_token_value = convert_token_to_digit(
      calibration_line.substring(i, calibration_line.length)
    );

    if (current_token_value != null) {
      if (first == null) {
        first = current_token_value;
      }

      last = current_token_value;
    }
  }
 ...
 * @returns number | null
 */
function convert_token_to_digit(token) {
  if (token[0] >= 0) {
    return +token[0];
  }

  const numeric_name_value_pairs = [
    { str: "zero", val: 0 },
    { str: "one", val: 1 },
    { str: "two", val: 2 },
    { str: "three", val: 3 },
  ...
  for (let i = 0; i < numeric_name_value_pairs.length; i++) {
    const number_name = numeric_name_value_pairs[i].str;
    if (token.substring(0, number_name.length) === number_name) {
      return numeric_name_value_pairs[i].val;
    }
  }
