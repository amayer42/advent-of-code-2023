const game_input = `Game 1: 3 blue, 7 green, 10 red; 4 green, 4 red; 1 green, 7 blue, 5 red; 8 blue, 10 red; 7 blue, 19 red, 1 green
...
const games = game_input.split("\n").map((line) => {
  const name_content_pair = line.split(":");
  let green_total = 0;
  let blue_total = 0;
  let red_total = 0;
  const sets = name_content_pair[1].split(";").map((set) => {
    const cubes = set.split(",");
    let green = 0;
    let blue = 0;
    let red = 0;
    for (let i = 0; i < cubes.length; i++) {
      const num_of_color = +cubes[i].trim().split(" ")[0];
      if (cubes[i].includes("green")) {
        green += num_of_color;
        green_total += num_of_color;
      }

      if (cubes[i].includes("blue")) {
        blue += num_of_color;
        blue_total += num_of_color;
      }

      if (cubes[i].includes("red")) {
        red += num_of_color;
        red_total += num_of_color;
      }
    }
  ...
  return {
    number: +name_content_pair[0].split(" ")[1],
    sets,
    max: {
      green: Math.max(...sets.map((s) => s.green)),
      blue: Math.max(...sets.map((s) => s.blue)),
      red: Math.max(...sets.map((s) => s.red)),
    },
    total: {
      green: green_total,
      blue: blue_total,
...
for (let i = 0; i < games.length; i++) {
  if (is_game_possible_with(games[i], 13, 14, 12)) {
    sum += games[i].number;
  }

  power_sum += games[i].max.green * games[i].max.blue * games[i].max.red;
}
console.log("Sum", sum);
console.log("Power Sum", power_sum);

function is_game_possible_with(game, green, blue, red) {
  let enough_green = false;
  let enough_blue = false;
  let enough_red = false;

  if (game.max.green <= green) {
    enough_green = true;
  }

  if (game.max.blue <= blue) {
    enough_blue = true;
  }
  ...
  return enough_green && enough_blue && enough_red;
}
